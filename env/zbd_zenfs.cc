    ...
    if (zbd_zone_type(z) == ZBD_ZONE_TYPE_SWR) {
      if (!zbd_zone_offline(z)) {
        ...
        if (zbd_zone_imp_open(z) || zbd_zone_exp_open(z) ||
            zbd_zone_closed(z)) {
          active_io_zones_++;
          if (zbd_zone_imp_open(z) || zbd_zone_exp_open(z)) newZone->Close();
        }
      }
    }
  }
...
Zone *ZonedBlockDevice::AllocateZone(Env::WriteLifeTimeHint file_lifetime) {
  ...
  if (best_diff >= LIFETIME_DIFF_NOT_GOOD) {
    ...
    for (const auto z : io_zones) {
      if ((!z->open_for_write_) && z->IsEmpty()) {
        ...
        active_io_zones_++; /* Do we need to protect this? */
        new_zone = 1;
        break;
      }
    }
  ...
  if (allocated_zone) {
    ...
    open_io_zones_++; /* Do we need to protect this? */
    Debug(logger_,
